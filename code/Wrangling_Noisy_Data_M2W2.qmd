...
# always standardize text when you need files to interact with each other and when doing modifications on the basis of given text
```
...
# Just as important to note when no individuals of a group are present than when in Ecology
```
...
# Modifications like this will be used a lot for poorly formated data sets
```
...
# Only use when analysis depends on the missing value
```
...
# Always standardize before merging data sets
```
