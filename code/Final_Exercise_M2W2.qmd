...
#### Check site locations (naming of sites not formatted for R)
...
#### Check sensor data (dates are in character format and legacy errors are present)  
...
#### Check species dictionary (only file formatted correctly)
...
#### Combine excel tabs on catch log into one data frame so all sheets load for catch log
#### Issues with catch log include inconsistent naming, wrong date format, and zero count entries missing
...
#### Put sensor data in daily format to combine with catch log
```{r}
...
Salinity naturally decreases further upstream 
...
Platycephalus seem to stick to Marine zones in line with being a marine species while Lates calcarifer/ Barramundi stick to the upstream which is expected as it is after spawning season when juvenile fish would be upstream
