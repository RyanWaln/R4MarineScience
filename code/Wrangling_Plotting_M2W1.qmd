...
## Pipe data to summarize mean body mass for each species and island and create a boxplot of body mass by species, with the island variable mapped to fill
```{r}
...
### Native R Binary RDS object: Ideal for storing clean, complex intermediate tables for your own subsequent workshops. Saving as an RDS file preserves your exact vector configurations (such as custom categorical factor levels and numeric type definitions), meaning you never have to re-format column types when you reload the data next week.
